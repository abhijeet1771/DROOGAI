        ...
        if (nameSimilarity > 0.7) {
          ...
          if (sigSimilarity > 0.6) {
            suggestions.push({
              ...
              suggestion: `Use existing method \`${mainSymbol.name}\` from \`${mainSymbol.file}\` instead of duplicating`,
              reason: `Method \`${mainSymbol.name}\` already exists with ${(sigSimilarity * 100).toFixed(0)}% similar signature`,
              existingLocation: `${mainSymbol.file}::${mainSymbol.name}`,
