...
async function runDeleteComments(options: any) {
  try {
    ...
    console.log('🗑️  Deleting all comments on PR...\n');
    console.log(`📦 Repository: ${owner}/${repo}`);
    console.log(`🔢 PR #${prNumber}\n`);

    const github = new GitHubClient(token);
    const result = await github.deleteAllPRComments(owner, repo, prNumber);

    console.log(`\n✅ Successfully deleted ${result.reviewComments + result.issueComments} comment(s)!\n`);
  } catch (error: any) {
    handleError(error);
  }
}
