...
export class EnterpriseReviewer {
  ...
  async reviewPR(prData: PRData, useIndex: boolean = false, geminiKey?: string, owner?: string, repo?: string, autoFix: boolean = false): Promise<EnterpriseReviewReport> {
    ...
    const deduplicatedAnalyzerComments = commentConverter.deduplicateComments(analyzerComments);
    ...
    // Merge analyzer comments with LLM comments and locator comments
    const allComments = [...(basicReport.comments || []), ...sortedAnalyzerComments, ...locatorComments];
    const finalComments = commentConverter.deduplicateComments(allComments);
    ...
    if (violations.length > 0) {
      ...
      enterpriseReport.comments = commentConverter.deduplicateComments(enterpriseReport.comments);
