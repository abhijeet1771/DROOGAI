...
export class ImpactAnalyzer {
  ...
  /**
   * Infer feature name from file path (enhanced for TypeScript/JavaScript)
   */
  private inferFeatureName(filepath: string): string {
    ...
    // e.g., "pages/LoginPage.ts" -> "Login"
    // e.g., "services/AuthService.ts" -> "Auth"
    
    const pathParts = filepath.split('/');
    const filename = pathParts[pathParts.length - 1];
    
    // Remove extension
    const nameWithoutExt = filename.replace(/\.(java|js|ts|tsx|jsx|py|go)$/, '');
    
    // TypeScript/JavaScript patterns: Page, Component, Service, etc.
    const patterns = [
      { pattern: /Page$/, remove: 'Page' },
      { pattern: /Component$/, remove: 'Component' },
      { pattern: /Service$/, remove: 'Service' },
      { pattern: /Controller$/, remove: 'Controller' },
      { pattern: /Manager$/, remove: 'Manager' },
      { pattern: /Handler$/, remove: 'Handler' },
      { pattern: /Helper$/, remove: 'Helper' },
      { pattern: /Util$/, remove: 'Util' },
      { pattern: /Utils$/, remove: 'Utils' },
    ];
    
    for (const { pattern, remove } of patterns) {
      if (pattern.test(nameWithoutExt)) {
        const featureName = nameWithoutExt.replace(pattern, '').trim();
        if (featureName) {
          return featureName;
        }
      }
    ...
    // Check parent directory (common in TypeScript/JavaScript projects)
    if (pathParts.length > 1) {
      const parentDir = pathParts[pathParts.length - 2];
      const ignoredDirs = ['src', 'com', 'org', 'lib', 'dist', 'build', 'node_modules', 'test', 'tests', 'spec', 'specs'];
      if (parentDir && !ignoredDirs.includes(parentDir.toLowerCase())) {
  ...
  private async predictBreakage(
    ...
    try {
      ...
      // Use gemini-2.5-flash to match other parts of codebase
