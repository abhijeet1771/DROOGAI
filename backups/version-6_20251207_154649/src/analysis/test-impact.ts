...
export class TestImpactAnalyzer {
  ...
  /**
   * Predict which tests will fail (enhanced with advanced prediction)
   */
  async predictFailingTests(changedSymbols: CodeSymbol[]): Promise<TestImpact[]> {
    const predictions: TestImpact[] = [];
    const index = this.indexer.getIndex();

    // Use advanced predictor if available
    try {
      const { AdvancedTestPredictor } = await import('./advanced-test-prediction.js');
      const advancedPredictor = new AdvancedTestPredictor(this.indexer);
      const testFiles = this.getAllTestFiles(index);
      const advancedPredictions = advancedPredictor.predictAdvanced(changedSymbols, testFiles);
      
      // Convert advanced predictions to TestImpact format
      return advancedPredictions.map(p => ({
        testFile: p.testFile,
        testMethod: p.testMethod,
        probability: p.probability,
        reason: `${p.reason} (Confidence: ${(p.confidence * 100).toFixed(0)}%)`,
        affectedCode: p.affectedCode,
        suggestion: p.suggestion,
      }));
    } catch (error) {
      // Fallback to basic prediction
      console.warn('Advanced test prediction not available, using basic prediction');
    }
  ...
  /**
   * Get all test files from index
   */
  private getAllTestFiles(index: any): string[] {
    const testFiles = new Set<string>();
    const symbols = index.symbols || [];
    for (const symbol of symbols) {
      if (this.isTestFile(symbol.file)) {
