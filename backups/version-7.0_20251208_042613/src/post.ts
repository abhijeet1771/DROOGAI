...
export class CommentPoster {
  ...
  async postComments(comments: ReviewComment[]): Promise<{ posted: ReviewComment[]; skipped: ReviewComment[] }> {
    ...
    // Post comments priority-wise
    // No limit for critical/high severity comments - show all critical issues
    for (const [file, fileComments] of commentsByFile) {
      if (fileComments.length > 0) {
        // Separate critical/high from medium/low
        const criticalHigh = fileComments.filter(c => 
          (c.severity || 'medium').toLowerCase() === 'high' || 
          (c.severity || 'medium').toLowerCase() === 'critical'
        );
        const mediumLow = fileComments.filter(c => 
          (c.severity || 'medium').toLowerCase() !== 'high' && 
          (c.severity || 'medium').toLowerCase() !== 'critical'
        );
        
        // Post ALL critical/high comments (no limit)
        // For medium/low: Only post if it's architecture or important issues, otherwise skip to summary
        // Limit: Max 2-3 medium comments per file (only if really important)
        const importantMedium = mediumLow.filter(c => {
          const msg = (c.message || '').toLowerCase();
          // Keep architecture violations and important issues
          return msg.includes('architecture') || 
                 msg.includes('solid') || 
                 msg.includes('dependency') ||
                 msg.includes('breaking');
        });
        const mediumLowToPost = importantMedium.slice(0, 3); // Max 3 important medium per file
        const mediumLowSkipped = [...mediumLow.filter(c => !importantMedium.includes(c)), ...importantMedium.slice(3)];
        
        const commentsToPost = [...criticalHigh, ...mediumLowToPost];
        const commentsSkipped = mediumLowSkipped;
        ...
        if (commentsSkipped.length > 0) {
          console.log(`  ⚠️  ${commentsSkipped.length} medium/low severity comment(s) skipped (will be shown in PR summary)`);
        }
        if (criticalHigh.length > 0) {
          console.log(`  ✓ Posted ${criticalHigh.length} critical/high severity comment(s) (no limit)`);
        }
      }
    }
  ...
  private formatHumanLikeComment(comment: ReviewComment): string {
    ...
    if (suggestion) {
      ...
      const fileExt = comment.file.split('.').pop()?.toLowerCase() || 'java';
      ...
      const lang = langMap[fileExt] || 'java';
      // Format as single code block
      formatted += `\n\`\`\`${lang}\n${cleanedSuggestion}\n\`\`\`\n`;
    }
   ...
   * But detect if imports are needed and add hint
   */
  private cleanCodeSuggestion(suggestion: string, issueLine: number): string {
    ...
    // 1. Extract import statements before removing them
    const importPattern = /^import\s+(?:static\s+)?([\w.*]+)\s*;?\s*$/gm;
    let match;
    while ((match = importPattern.exec(suggestion)) !== null) {
      detectedImports.push(match[0].trim());
    }
    
    // 2. Remove import statements (they belong at file top, not in method body)
    cleaned = cleaned.replace(/^import\s+(?:static\s+)?[\w.*]+\s*;?\s*$/gm, '');
    
    // 3. Remove any lines that are just imports (handle multi-line)
    ...
    // 4. If imports were detected, add hint at the end
    if (detectedImports.length > 0) {
      cleaned += '\n\n';
      cleaned += '**Note:** Add these imports at the top of the file:\n';
