...
**IMPORTANT - Communication Style (Human-like, Conversational):**
- Write as a helpful senior colleague, not a robot or linter
- Use natural, conversational language - like you're explaining to a teammate over coffee
- Show reasoning: "I noticed X, which could cause Y because Z"
- Provide context: "This pattern is common in our codebase, but here's a better approach"
- Give examples: "Similar to how we handle this in ServiceA, you could..."
- Be empathetic: "I understand the intent, but there's a safer way to do this"
- Focus on impact: "If this runs in production, it will fail when..." or "This will break the Login feature because..."
- Explain the "why" behind suggestions, not just the "what"
- Use "you" naturally: "You might want to consider..." instead of "Consider..."
- All comments must be in English
- Vary your phrasing - don't use the same template for every comment
- Be concise but warm - professional yet friendly
...
[
  {
    ...
    "message": "Natural, conversational explanation. Show reasoning and context. Example format: 'I noticed [issue] which could cause [impact] because [reason]. In our codebase, we typically handle this by [pattern]. Here's a safer approach: [suggestion].' Be empathetic and helpful, like explaining to a teammate.",
...
export class LLMReviewer {
  private model: any;
  private intelligentFallback: any = null;

  constructor(apiKey: string) {
    const genAI = new GoogleGenerativeAI(apiKey);
    this.model = genAI.getGenerativeModel({ model: 'gemini-2.5-flash' });
    
    // Initialize intelligent fallback (lazy load to avoid dependency issues)
    this.initializeFallback();
  }

  /**
   * Initialize intelligent fallback (async, non-blocking)
   */
  private async initializeFallback(): Promise<void> {
    try {
      // Dynamic import to avoid TypeScript compilation issues
      const module = await import('../fallbacks/intelligent-comment-generator.js' as string);
      const IntelligentCommentGenerator = (module as any).IntelligentCommentGenerator;
      if (IntelligentCommentGenerator) {
        this.intelligentFallback = new IntelligentCommentGenerator();
      }
    } catch (error) {
      // Fallback not available - continue without it
      this.intelligentFallback = null;
    }
  }

  /**
   * Extract code from diff (for fallback analysis)
   */
  private extractCodeFromDiff(diff: string): string {
    ...
    for (const line of lines) {
      ...
      // Include context lines (starting with space)
...
Analyze this diff and return JSON array of issues.`;
    ...
    try {
      ...
      // Sanitize JSON: fix common escape character issues
      let sanitizedJson = jsonMatch[0];
      
      let comments: ReviewComment[] = [];
      try {
        comments = JSON.parse(sanitizedJson);
      } catch (parseError: any) {
        console.warn(`JSON parse error for ${filepath}: ${parseError.message}`);
        console.warn(`Attempting to fix JSON...`);
        ...
        try {
          ...
          // Only replace single quotes that are likely JSON delimiters (at start/end of values)
          ...
          sanitizedJson = sanitizedJson.replace(/\\(?!["\\/bfnrtu0-9])/g, '\\\\');
          
          comments = JSON.parse(sanitizedJson);
        } catch (secondError: any) {
          console.error(`Failed to parse JSON for ${filepath} after fixes: ${secondError.message}`);
          console.error(`JSON snippet (first 500 chars): ${sanitizedJson.substring(0, 500)}`);
          return [];
        }
      }
      ...
      // Check for daily quota limit (different from rate limit)
      const isDailyQuota = errorMessage.includes('Quota exceeded') || 
                          errorString.includes('free_tier_requests') ||
                          errorString.includes('GenerateRequestsPerDayPerProjectPerModel-FreeTier');
      
      if (isDailyQuota) {
        // Daily quota exhausted - use intelligent fallback
        console.error(`\n❌ Daily Quota Exceeded for Gemini API`);
        console.error(`   📊 Free tier limit: 20 requests/day per model`);
        console.error(`   ⏰ Quota resets in ~24 hours from first request`);
        console.error(`\n💡 Using intelligent fallback (ESLint + Semgrep + Pattern Analysis)...`);
        
        // Try intelligent fallback
        if (this.intelligentFallback) {
          try {
            const fallbackComments = await this.intelligentFallback.generateComments(filepath, this.extractCodeFromDiff(diff), diff);
            if (fallbackComments.length > 0) {
              console.log(`   ✓ Generated ${fallbackComments.length} comment(s) using intelligent fallback`);
              return fallbackComments;
            }
          } catch (fallbackError: any) {
            console.warn(`   ⚠️  Fallback generation failed: ${fallbackError.message}`);
          }
        }
        
        console.error(`\n⚠️  Skipping AI review for ${filepath}. Static analysis results will still be available.`);
        return [];
      }
      ...
      // Connection error handling - try fallback
      if (errorMessage.includes('fetch') || errorMessage.includes('network') || errorMessage.includes('ECONNREFUSED')) {
        console.error(`Error reviewing ${filepath}:`, errorMessage);
        console.error('   ⚠️  Connection issue with Gemini API. Trying intelligent fallback...');
  ...
  async reviewBatchWithContext(
    ...
    for (let chunkIndex = 0; chunkIndex < chunks.length; chunkIndex++) {
      ...
      prompt += `\n\n**Remember:** Use natural, conversational language in your own style. Focus on impact: what will break, where, why. Be respectful and helpful.`;
      ...
      try {
        ...
        let comments: ReviewComment[] = [];
        try {
          comments = JSON.parse(sanitizedJson) as ReviewComment[];
        } catch (parseError: any) {
          console.warn(`⚠️  Batch ${chunkIndex + 1} failed, falling back to individual reviews: ${parseError.message}`);
          // Try to fix JSON before falling back
          try {
            ...
            for (let i = 0; i < chunk.length; i++) {
              const file = chunk[i];
              const fileComments = await this.reviewDiff(file.file, file.diff);
