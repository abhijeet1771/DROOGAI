...
import { MultiLanguageExtractor } from '../parser/multi-language-extractor.js';
...
export class EnterpriseReviewer {
  ...
  private multiLanguageExtractor: MultiLanguageExtractor;
  ...
  constructor(geminiKey: string, githubToken: string) {
    ...
    this.multiLanguageExtractor = new MultiLanguageExtractor();
  ...
  async reviewPR(prData: PRData, useIndex: boolean = false, geminiKey?: string, owner?: string, repo?: string, autoFix: boolean = false): Promise<EnterpriseReviewReport> {
    ...
    for (const file of prData.files) {
      if (file.patch) {
        ...
        if (code) {
          ...
          const parsed = await this.multiLanguageExtractor.extractFromCode(code, file.filename);
    ...
    const failingTests = await testImpactAnalyzer.predictFailingTests(prSymbols);
    ...
    // Logic Bug Detection
    console.log('\n📋 Phase 0.17.5: Logic Bug Detection...');
    const { LogicBugAnalyzer } = await import('../analysis/logic-bugs.js');
    const logicBugAnalyzer = new LogicBugAnalyzer();
    const allLogicBugs: any[] = [];
    for (const [filename, code] of prFileContents.entries()) {
      const fileSymbols = prSymbols.filter(s => s.file === filename);
      const logicBugReport = logicBugAnalyzer.analyzeLogicBugs(fileSymbols, code, filename);
      allLogicBugs.push(...logicBugReport.bugs);
    }
    console.log(`✓ Found ${allLogicBugs.length} logic bug(s): ${allLogicBugs.filter(b => b.severity === 'critical').length} critical, ${allLogicBugs.filter(b => b.severity === 'high').length} high`);

    // Direct Analyzer-to-Comment Conversion (will be done after security/performance analysis)
    ...
    for (const [filename, code] of prFileContents.entries()) {
      const fileSymbols = prSymbols.filter(s => s.file === filename);
      const perfReport = this.performanceAnalyzer.analyzePerformance(fileSymbols, code, filename);
    ...
    for (const [filename, code] of prFileContents.entries()) {
      const fileSymbols = prSymbols.filter(s => s.file === filename);
      const secReport = this.securityAnalyzer.analyzeSecurity(fileSymbols, code, filename);
      // Set file names
    ...
    // Direct Analyzer-to-Comment Conversion
    console.log('\n📋 Phase 0.17.6: Direct Analyzer-to-Comment Conversion...');
    const { AnalyzerToCommentConverter } = await import('./analyzer-to-comments.js');
    const commentConverter = new AnalyzerToCommentConverter();
    // Set PR files for line number mapping
    commentConverter.setPRFiles(prData.files);
    // Set Gemini key for LLM-based comment merging
    if (geminiKey) {
      await commentConverter.setGeminiKey(geminiKey);
    }
    
    // Convert all analyzer results to comments
    const analyzerComments: ReviewComment[] = [];
    
    // Convert logic bugs
    if (allLogicBugs.length > 0) {
      const logicBugComments = commentConverter.convertLogicBugs(allLogicBugs);
      analyzerComments.push(...logicBugComments);
      console.log(`  ✓ Converted ${logicBugComments.length} logic bug(s) to comments`);
    }
    
    // Convert security issues
    if (allSecurityIssues.length > 0) {
      const securityComments = commentConverter.convertSecurityIssues(allSecurityIssues);
      analyzerComments.push(...securityComments);
      console.log(`  ✓ Converted ${securityComments.length} security issue(s) to comments`);
    }
    
    // Convert performance issues
    if (allPerformanceIssues.length > 0) {
      const performanceComments = commentConverter.convertPerformanceIssues(allPerformanceIssues);
      analyzerComments.push(...performanceComments);
      console.log(`  ✓ Converted ${performanceComments.length} performance issue(s) to comments`);
    }
    
    // Deduplicate and sort (with LLM merging)
    const deduplicatedAnalyzerComments = await commentConverter.deduplicateComments(analyzerComments);
    const sortedAnalyzerComments = commentConverter.sortByPriority(deduplicatedAnalyzerComments);
    console.log(`  ✓ Total analyzer comments: ${sortedAnalyzerComments.length} (after deduplication)`);
    ...
    for (const [filepath, code] of prFileContents.entries()) {
      // Check if it's a test file - improved detection
      const isTestFile = filepath.toLowerCase().includes('test') || 
                         ...
                         filepath.match(/test.*\.(ts|js|tsx|jsx)$/i) ||
                         filepath.match(/.*\.(test|spec)\.(ts|js|tsx|jsx)$/i);
      
      if (isTestFile) {
        // Analyze locators (with AI enhancement if geminiKey available)
        const locatorAnalyzerWithAI = new LocatorSuggestionAnalyzer((this as any).geminiKey);
        try {
          const locatorAnalysis = await locatorAnalyzerWithAI.analyzeLocators(code, filepath);
          if (locatorAnalysis && locatorAnalysis.suggestions && locatorAnalysis.suggestions.length > 0) {
            locatorSuggestions.push(...locatorAnalysis.suggestions);
          }
        } catch (error) {
          console.warn(`⚠️  Locator analysis failed for ${filepath}:`, (error as any).message);
        }
        
        // Analyze Gherkin/feature files
        if (filepath.endsWith('.feature') || filepath.toLowerCase().includes('feature') || filepath.toLowerCase().includes('step')) {
          try {
            const gherkinAnalysis = gherkinAnalyzer.analyzeGherkin(code, filepath);
            if (gherkinAnalysis && gherkinAnalysis.suggestions && gherkinAnalysis.suggestions.length > 0) {
              gherkinSuggestions.push(...gherkinAnalysis.suggestions);
            }
          } catch (error) {
            console.warn(`⚠️  Gherkin analysis failed for ${filepath}:`, (error as any).message);
          }
        }
      }
    }
    ...
    // Convert locator suggestions to inline comments
    let locatorComments: ReviewComment[] = [];
    if (locatorSuggestions.length > 0) {
      locatorComments = commentConverter.convertLocatorSuggestions(locatorSuggestions);
      console.log(`  ✓ Converted ${locatorComments.length} locator suggestion(s) to inline comments`);
    }

    // Architecture comments will be added later in Phase 6
    let architectureComments: ReviewComment[] = [];

    // Merge analyzer comments with LLM comments and locator comments (with LLM merging)
    const allComments = [...(basicReport.comments || []), ...sortedAnalyzerComments, ...locatorComments];
    const finalComments = await commentConverter.deduplicateComments(allComments);
    const sortedFinalComments = commentConverter.sortByPriority(finalComments);
    
    const enterpriseReport: EnterpriseReviewReport = {
      ...basicReport,
      comments: sortedFinalComments, // Use merged comments
      duplicates: { 
      ...
      logicBugs: {
        bugs: allLogicBugs,
        critical: allLogicBugs.filter(b => b.severity === 'critical'),
        high: allLogicBugs.filter(b => b.severity === 'high'),
        medium: allLogicBugs.filter(b => b.severity === 'medium'),
        low: allLogicBugs.filter(b => b.severity === 'low'),
      },
      impactAnalysis: {
    ...
    // Convert architecture violations to inline comments
    if (violations.length > 0) {
      const archComments = commentConverter.convertArchitectureViolations(violations);
      console.log(`  ✓ Converted ${archComments.length} architecture violation(s) to inline comments`);
      
      // Add architecture comments to existing comments
      enterpriseReport.comments.push(...archComments);
      // Re-deduplicate and sort (with LLM merging)
      enterpriseReport.comments = await commentConverter.deduplicateComments(enterpriseReport.comments);
      enterpriseReport.comments = commentConverter.sortByPriority(enterpriseReport.comments);
    }
  ...
  private generateSummary(report: EnterpriseReviewReport): string {
    ...
    if (report.impactAnalysis && report.impactAnalysis.impactedFiles.length > 0) {
      ...
      if (report.impactAnalysis.impactedFeatures.length > 0) {
        ...
        const sortedFeatures = [...report.impactAnalysis.impactedFeatures].sort((a, b) => {
          const aRisk = riskOrder[a.riskLevel?.toLowerCase() as keyof typeof riskOrder] ?? 2;
          const bRisk = riskOrder[b.riskLevel?.toLowerCase() as keyof typeof riskOrder] ?? 2;
          return aRisk - bRisk;
        });
    ...
    // Separate section for Predicted Breakage Scenarios (future potential issues)
    if (report.impactAnalysis && report.impactAnalysis.breakagePredictions.length > 0) {
      summary += `## 🔮 Predicted Breakage Scenarios (Future Risks)\n\n`;
      summary += `These are potential issues that might occur in the future, not immediate breaking changes:\n\n`;
      
      // Sort by probability: HIGH → MEDIUM → LOW
      const probOrder = { 'high': 0, 'medium': 1, 'low': 2 };
      const sortedPredictions = [...report.impactAnalysis.breakagePredictions].sort((a, b) => {
        const aProb = probOrder[a.probability?.toLowerCase() as keyof typeof probOrder] ?? 2;
        const bProb = probOrder[b.probability?.toLowerCase() as keyof typeof probOrder] ?? 2;
        return aProb - bProb;
      });
      
      sortedPredictions.forEach((pred: any) => {
        summary += `### ${pred.scenario} (${pred.probability.toUpperCase()} probability)\n`;
        summary += `- **Potential impact:** ${pred.impact}\n`;
        summary += `- **Files at risk:** ${pred.affectedFiles.join(', ')}\n`;
    ...
    if (report.impactAnalysis && report.impactAnalysis.impactedFeatures.length > 0) {
      // Sort features by risk level: HIGH → MEDIUM → LOW
      const riskOrder = { 'high': 0, 'medium': 1, 'low': 2 };
      const sortedFeatures = [...report.impactAnalysis.impactedFeatures].sort((a, b) => {
        const aRisk = riskOrder[a.riskLevel?.toLowerCase() as keyof typeof riskOrder] ?? 2;
        const bRisk = riskOrder[b.riskLevel?.toLowerCase() as keyof typeof riskOrder] ?? 2;
      ...
      // Add high and medium risk features to must fix
      sortedFeatures.forEach((feature: any) => {
        if (feature.riskLevel?.toLowerCase() === 'high' || feature.riskLevel?.toLowerCase() === 'medium') {
          const priority = feature.riskLevel?.toLowerCase() === 'high' ? '🔴 HIGH' : '🟡 MEDIUM';
          mustFix.push({
            priority,
            issue: `Fix ${feature.name} feature`,
            impact: `${feature.files.length} file(s) affected - ${feature.riskLevel.toUpperCase()} RISK`
          });
        }
      });
    }
    
    // Sort mustFix by priority: HIGH → MEDIUM
    ...
    if (report.codebaseKnowledge) {
      const totalReusableMethods = report.codebaseKnowledge.reusableMethods?.length || 0;
      const totalSuggestions = report.codebaseKnowledge.suggestions?.length || 0;
      const displayCount = Math.max(totalReusableMethods, totalSuggestions);
      
      if (displayCount > 0) {
        summary += `\n## 💡 Codebase Knowledge (Reuse Opportunities)\n\n`;
        summary += `💡 **Codebase Knowledge:** ${displayCount} reuse opportunity(ies)\n\n`;
        
        if (report.codebaseKnowledge.summary) {
          summary += `${report.codebaseKnowledge.summary}\n\n`;
        }
      }
      
      if (report.codebaseKnowledge.reusableMethods && report.codebaseKnowledge.reusableMethods.length > 0) {
        ...
        // Display deduplicated methods - show all, not just first 5
        let methodIndex = 1;
        const allMethods = Array.from(deduplicatedMethods.values());
        ...
        if (allMethods.length < totalReusableMethods) {
          summary += `*... and ${totalReusableMethods - allMethods.length} more reusable method(s)*\n\n`;
        }
      }
