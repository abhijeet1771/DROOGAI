...
export class PerformanceAnalyzer {
  ...
  analyzePerformance(symbols: CodeSymbol[], codeContent: string, filepath?: string): PerformanceReport {
    ...
    for (const symbol of symbols) {
      // Check both 'method' and 'function' types (for TypeScript/JavaScript)
  ...
  private detectInefficientLoops(
    ...
    if (/for\s*\(|while\s*\(|\.forEach\s*\(/.test(methodCode)) {
      ...
      if (hasStringConcat && !/StringBuilder|StringBuffer|String\.join|array\.join|\.join\(|str\.join/.test(methodCode)) {
        // Find the exact line with string concatenation
        let concatLineNumber = symbol.startLine;
        for (let i = 0; i < methodLines.length; i++) {
          const line = methodLines[i];
          const isComment = line.trim().startsWith('//') || line.trim().startsWith('/*') || line.trim().startsWith('*');
          if (!isComment) {
            if (language === 'typescript' || language === 'javascript') {
              if (/(\w+)\s*\+=\s*|(\w+)\s*=\s*(\w+)\s*\+\s*/.test(line) && !/array\.join|\.join\(|template|`/.test(line)) {
                concatLineNumber = symbol.startLine + i;
                break;
              }
            } else if (language === 'java' || language === 'unknown') {
              if (/(\w+)\s*\+=\s*["']|String\s+\w+\s*\+=\s*/.test(line) && !/StringBuilder|StringBuffer|String\.join/.test(line)) {
                concatLineNumber = symbol.startLine + i;
                break;
              }
            }
        ...
        issues.push({
          type: 'Inefficient Loop',
          location: `${symbol.file}:${concatLineNumber}`,
          file: symbol.file,
          line: concatLineNumber,
          method: symbol.name,
          severity: 'medium',
